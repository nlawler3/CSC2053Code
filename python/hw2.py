# NATALIE LAWLER
# I used a website called "spark by examples" to find out how to get unique values from a list using the set() function
...
def remove_range(alist, min, max):
   ...
   *** Important: your code must use comprehensions and should not be more than
   two lines of code including the return statement ***
    """
    new_list = [value for value in alist if value > max or value < min]
    return new_list
...
def word_count_in_set(words):
    ...
    *** Solution should not be more than 3 lines of code (can be less)
    ...
    """
    words_set = set(words) # gets unique value
...
def word_count(strings):
    ...
    *** Important: your code must use comprehensions and should not be more than two
    ...
    """
    dictionary_result = {char: strings.count(char) for char in strings}
...
def friend_list(friend_dictionary):
    ...
    """
    new_dictionary = {}
    for person_name, friend_name in friend_dictionary.items():
        if friend_name not in new_dictionary:
            new_dictionary[friend_name] = []
        if person_name not in new_dictionary:
            new_dictionary[person_name] = []
        new_dictionary[person_name].append(friend_name)
        new_dictionary[friend_name].append(person_name)
